 ...
 */
import { CfgGraphViewer } from './cfg-viewer.js?v=20260923-pi-example';


// JS/WASM 必须使用同一缓存版本，修改并重新构建 WASM 后应同步更新此值。
const CIFA_ASSET_VERSION = '20260923-pi-example';
...
const EXAMPLE_GROUPS = [
    ...
    },
    {
        ...
        items: [
            ...
            { id: 'function_overloading', name: '函数重载', code: `// 同名函数可按参数个数重载\ndescribe() {\n    println("无参数");\n    return 0;\n}\n\ndescribe(value) {\n    println("一个参数: ", value);\n    return value;\n}\n\ndescribe(left, right) {\n    println("两个参数: ", left, ", ", right);\n    return left + right;\n}\n\nreturn describe() + describe(2) + describe(3, 4);` },
            { id: 'for_range', name: 'for-range 循环', code: `values = {1, 2, 3, 4, 5};\n\nprintln("--- int 遍历 ---");\nint sum = 0;\nfor (int v : values) {\n    if (v == 3) continue;\n    if (v == 5) break;\n    sum += v;\n    println("  v = ", v);\n}\nprintln("sum = ", sum);\n\nprintln("--- auto 遍历 ---");\nint product = 1;\nfor (auto v : values) {\n    product *= v;\n}\nprintln("product = ", product);\n\nreturn sum;` },
    ...
    },
    {
        ...
        items: [
            ...
            { id: 'string_compare', name: '字符串比较', code: `string a = "hello";\nstring b = "world";\nstring c = "hello";\n\nprintln("a == c: ", a == c);\nprintln("a != b: ", a != b);\nprintln("a == b: ", a == b);\n\nif ("abc" == "abc" && "abc" != "def") {\n    println("字符串 == 和 != 正常工作");\n}\n\nreturn a == c ? 1 : 0;` },
...
auto big_add(a, b) {
    ...
    for (int i = 0; i < max_len || carry > 0; i++) {
        ...
        double sum = (double)val_a + val_b + carry;
        res.push_back(floor(fmod(sum, BASE)));
        carry = floor(sum / BASE);
    }
    return res;
}

// 大数减法: res = a - b (前提要求 a >= b，基于 Base 10^9)
auto big_sub(a, b) {
    res = {};
    int len_a = size(a);
    int len_b = size(b);
...
// 大数乘单个整数: res = a * factor (基于 Base 10^9)
auto big_mul_int(a, factor) {
    ...
    for (int i = 0; i < len || carry > 0; i++) {
        double val = carry;
        if (i < len) val += (double)a[i] * factor;
        res.push_back(floor(fmod(val, BASE)));
        carry = floor(val / BASE);
    }
    return res;
}

// 大数除以单个整数: res = a / divisor (基于 Base 10^9)
...
// 500/9 约 56 个块 + 3 缓冲区 = 59 个块（原方案需要 128 个块，计算量减少一大半）
int num_blocks = floor(target_digits / 9) + 3;
...
class CifaPlayground {
    ...
    async initCifa() {
        try {
            const moduleUrl = new URL('../cifa.js', import.meta.url);
            moduleUrl.searchParams.set('v', CIFA_ASSET_VERSION);
            const { default: CifaModule } = await import(moduleUrl.href);

            this.cifaModule = await CifaModule({
                locateFile: (path) => {
                    const base = new URL('../', import.meta.url);
                    const url = new URL(path, base);
                    url.searchParams.set('v', CIFA_ASSET_VERSION);
                    return url.toString();
                }
            });
            this.isReady = true;
